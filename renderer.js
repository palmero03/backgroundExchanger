...
ipcRenderer.on('searchPorts', (event, data) => {
	user = data.user;
	pwd = data.password;
		...
		ports.forEach(function(port) {
			...
			sp.open(function (error) { 
				if ( !error ) { 
					...
					renderPorts('COM Port connection Test:<br/>' + portsStr + 'Waiting for Messages:<br/>');
...
function renderPorts(portsStr) {
	str = '<div class="card">'+
					'<div class="card-header">Logged as '+user+'</div>'+
					'<div class="card-body">'+
					portsStr +
					'</div>'+
				'</div>';
